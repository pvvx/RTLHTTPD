...
RAM1_IMAGE ?= $(BIN_DIR)/ram_1.r.bin
...
#test:
#	@$(OPENOCD) -f interface/$(FLASHER).cfg -c "transport select swd" -f $(FLASHER_PATH)rtl8710.ocd -c "init" -c "adapter_khz $(FLASHER_SPEED)" -c "reset halt" \
#	-c "load_image $(TST_IMAGE) $(LD_ADDRESS) bin" -c "cortex_bootstrap $(ST_ADDRESS)" -c "shutdown"
	
readfullflash:
	@$(OPENOCD) -f interface/$(FLASHER).cfg -c "transport select swd" -f $(FLASHER_PATH)rtl8710.ocd -c "init" -c "adapter_khz $(FLASHER_SPEED)" -c "reset halt" \
	-c "rtl8710_flash_read_id" \
	-c "rtl8710_flash_read $(BIN_DIR)/fullflash.bin 0 1048576" \
	-c "reset run" -c "shutdown"
